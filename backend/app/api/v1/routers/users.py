...
@router.post("/login")
async def login(response: Response, login_schema: LoginSchema, uow: UOWDep):
    try:
        user_service = UserService(uow)
        new_tokens = await user_service.login(email=login_schema.email, pwd=login_schema.pwd)
        ...
        logger.critical("Unknow error in login", exc_info=True, extra={"email": login_schema.email, "pwd": login_schema.pwd})
