...
class CollectionRepository(ICollectionRepository, BaseSQLAlchemyRepository[CollectionDTO, Collection]):
    ...
    async def collections_with_category_and_photo(self, **filters) -> list[tuple[Any]]:
        query = select(self.model).where(*[getattr(self.model, k) == v for k, v in filters.items()]).options(joinedload(self.model.photo),  joinedload(self.model.collection_category))
